...
struct Repackage : boost::static_visitor<TargetVariant> {
  template <typename T> TargetVariant operator()(const T &t) const {
...
std_cxx17::optional<std_cxx17::variant<CGALPoint2, CGALSegment2, CGALTriangle2,
...
compute_intersection_triangle_triangle(
  ...
  std::transform(triangle0.begin(), triangle0.end(), pts0.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);

  std::transform(triangle1.begin(), triangle1.end(), pts1.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);
...
compute_intersection_triangle_segment(
  ...
  std::transform(triangle.begin(), triangle.end(), pts0.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);

  std::transform(segment.begin(), segment.end(), pts1.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);

  CGALTriangle2 cgal_triangle{pts0[0], pts0[1], pts0[2]};
  CGALSegment2 cgal_segment{pts1[0], pts1[1]};
  return convert_boost_to_std(CGAL::intersection(cgal_segment, cgal_triangle));
}
...
compute_intersection_rect_rect(const ArrayView<const Point<2>> &rectangle0,
  ...
  std::transform(rectangle0.begin(), rectangle0.end(), pts0.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);

  std::transform(rectangle1.begin(), rectangle1.end(), pts1.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);
  ...
  CGAL::intersection(first_poly, second_poly, std::back_inserter(poly_list));
  return poly_list;
}

std_cxx17::optional<std_cxx17::variant<CGALPoint3, CGALSegment3>>
compute_intersection_tetra_segment(const ArrayView<const Point<3>> &tetrahedron,
  ...
  std::transform(tetrahedron.begin(), tetrahedron.end(), pts0.begin(),
...
// tetra, triangle
std_cxx17::optional<std_cxx17::variant<CGALPoint3, CGALSegment3, CGALTriangle3,
...
compute_intersection_tetra_triangle(
  ...
  std::transform(triangle.begin(), triangle.end(), pts1.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint3, 3>);
...
compute_intersection_quad_quad(const ArrayView<const Point<2>> &quad0,
  ...
  if (!intersection_test.empty()) {
    ...
    if (size_poly == 3) {
      ...
      // vertices.
      return {{{CGALWrappers::cgal_point_to_dealii_point<2>(poly.vertex(0)),
      ...
      for (Face_handle f : cdt.finite_face_handles()) {
        if (f->info().in_domain() &&
          ...
          collection.push_back({{CGALWrappers::cgal_point_to_dealii_point<2>(
...
compute_intersection_quad_line(const ArrayView<const Point<2>> &quad,
  ...
  std::transform(quad.begin(), quad.end(), pts.begin(),
                 &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);
  ...
  for (Face_handle f : cdt.finite_face_handles()) {
    if (f->info().in_domain() &&
      ...
      if (const CGALSegment2 *s = boost::get<CGALSegment2>(&*intersection)) {
        vertices.push_back(
    ...
    if (CGAL::do_intersect(cgal_segment, cgal_tetrahedron)) {
      ...
      if (const CGALSegment3_exact *s =
        ...
        if (s->squared_length() > tol * tol) {
          vertices.push_back(
              {{CGALWrappers::cgal_point_to_dealii_point<3>(s->vertex(0)),
                CGALWrappers::cgal_point_to_dealii_point<3>(s->vertex(1))}});
        }
      }
    }
  }
      ...
      if (CGAL::do_intersect(tet, triangulation_quad.triangle(f))) {
        ...
        if (const CGALTriangle3_exact *t =
          ...
          if (CGAL::to_double(t->squared_area()) > tol * tol) {
            vertices.push_back({{cgal_point_to_dealii_point<3>((*t)[0]),
...
compute_intersection_hexa_hexa(const ArrayView<const Point<3>> &hexa0,
  ...
  std::transform(
      hexa0.begin(), hexa0.end(), pts_hex0.begin(),
      &CGALWrappers::dealii_point_to_cgal_point<CGALPoint3_inexact, 3>);

  std::transform(
      hexa1.begin(), hexa1.end(), pts_hex1.begin(),
      &CGALWrappers::dealii_point_to_cgal_point<CGALPoint3_inexact, 3>);
  ...
  for (const auto &c0 : tria0.finite_cell_handles()) {
    ...
    const auto &tetg0 = CGAL::make_tetrahedron(
        tet0.vertex(0), tet0.vertex(1), tet0.vertex(2), tet0.vertex(3), surf0);
    (void)tetg0; // instead of C++ 17s [[maybe unused]]
    for (const auto &c1 : tria1.finite_cell_handles()) {
      const auto &tet1 = tria1.tetrahedron(c1);
      const auto &tetg1 =
          CGAL::make_tetrahedron(tet1.vertex(0), tet1.vertex(1), tet1.vertex(2),
                                 tet1.vertex(3), surf1);
      ...
      if (PMP::volume(sm) > tol && test_intersection) {
        ...
        triangulation_hexa.insert(sm.points().begin(), sm.points().end());
        for (const auto &c : triangulation_hexa.finite_cell_handles()) {
          const auto &tet = triangulation_hexa.tetrahedron(c);
          vertices.push_back(
...
compute_intersection_of_cells(const ArrayView<const Point<spacedim>> &vertices0,
  ...
  Assert(n_vertices0 > 0 || n_vertices1 > 0,
         ExcMessage("The intersection cannot be computed as at least one of "
                    "the two cells has no vertices."));

  if constexpr (dim0 == 2 && dim1 == 2 && spacedim == 2) {
    if (n_vertices0 == 4 && n_vertices1 == 4) {
      return internal::compute_intersection_quad_quad(vertices0, vertices1,
                                                      tol);
    }
  } else if constexpr (dim0 == 2 && dim1 == 1 && spacedim == 2) {
    if (n_vertices0 == 4 && n_vertices1 == 2) {
      return internal::compute_intersection_quad_line(vertices0, vertices1,
                                                      tol);
    }
  } else if constexpr (dim0 == 3 && dim1 == 1 && spacedim == 3) {
    if (n_vertices0 == 8 && n_vertices1 == 2) {
      return internal::compute_intersection_hexa_line(vertices0, vertices1,
                                                      tol);
    }
  } else if constexpr (dim0 == 3 && dim1 == 2 && spacedim == 3) {
    if (n_vertices0 == 8 && n_vertices1 == 4) {
      return internal::compute_intersection_hexa_quad(vertices0, vertices1,
                                                      tol);
    }
  } else if constexpr (dim0 == 3 && dim1 == 3 && spacedim == 3) {
    if (n_vertices0 == 8 && n_vertices1 == 8) {
      return internal::compute_intersection_hexa_hexa(vertices0, vertices1,
                                                      tol);
    }
  } else {
...
compute_intersection_of_cells(
    ...
    const Mapping<dim1, spacedim> &mapping1, const double tol) {
...
compute_intersection_of_cells<2, 1, 2>(
    ...
    const Mapping<2, 2> &mapping0, const Mapping<1, 2> &mapping1,
    const double tol);
...
compute_intersection_of_cells<2, 2, 2>(
    ...
    const Mapping<2, 2> &mapping0, const Mapping<2, 2> &mapping1,
    const double tol);
...
compute_intersection_of_cells<3, 2, 3>(
    ...
    const Mapping<3, 3> &mapping0, const Mapping<2, 3> &mapping1,
    const double tol);
...
compute_intersection_of_cells<3, 3, 3>(
    ...
    const Mapping<3, 3> &mapping0, const Mapping<3, 3> &mapping1,
    const double tol);

template std::vector<std::array<Point<2>, 1 + 1>>
compute_intersection_of_cells<2, 1, 2>(
    const ArrayView<const Point<2>> &vertices0,
    const ArrayView<const Point<2>> &vertices1, const double tol);
...
template std::vector<std::array<Point<3>, 2 + 1>>
compute_intersection_of_cells<3, 2, 3>(
    const ArrayView<const Point<3>> &vertices0,
    const ArrayView<const Point<3>> &vertices1, const double tol);
...
template <int dim> struct QGaussSimplexExt : public QGaussSimplex<dim> {

  QGaussSimplexExt(const unsigned int degree) : QGaussSimplex<dim>{degree} {};

  template <int spacedim>
  Quadrature<spacedim> compute_affine_transformation(
    ...
    for (unsigned int i = 0; i < this->size(); ++i) {
      qp[i] = Point<spacedim>(vertices[0] +
                              apply_transformation(B, this->point(i)));
      w[i] = J * this->weight(i);
    }
  ...
  template <int spacedim>
  Quadrature<spacedim> mapped_quadrature(
      const std::vector<std::array<Point<spacedim>, dim + 1>> &simplices)
      const {
    ...
    for (const auto &simplex : simplices) {
      const auto rule = compute_affine_transformation(simplex);
      std::transform(rule.get_points().begin(), rule.get_points().end(),
                     std::back_inserter(qp),
                     [&](const Point<spacedim> &p) { return p; });
      std::transform(rule.get_weights().begin(), rule.get_weights().end(),
                     std::back_inserter(ws), [&](const double w) { return w; });
    }
    return Quadrature<spacedim>(qp, ws);
  }
};

// Routines for cells intersection
template <>
Quadrature<1> compute_quadrature_on_intersection<1, 1, 1>(
    const typename Triangulation<1, 1>::cell_iterator &cell0,
    const typename Triangulation<1, 1>::cell_iterator &cell1,
    const unsigned int degree, const Mapping<1, 1> &mapping0,
    const Mapping<1, 1> &mapping1, const double tol) {
  Assert(false, ExcNotImplemented());
  (void)cell0;
  (void)cell1;
  (void)degree;
...
template <>
Quadrature<2> compute_quadrature_on_intersection<2, 1, 2>(
    const typename Triangulation<2, 2>::cell_iterator &cell0,
    const typename Triangulation<1, 2>::cell_iterator &cell1,
    const unsigned int degree, const Mapping<2, 2> &mapping0,
    const Mapping<1, 2> &mapping1, const double tol) {

  const std::vector<std::array<Point<2>, 2>> &vec_of_simplices =
      CGALWrappers::compute_intersection_of_cells(cell0, cell1, mapping0,
                                                  mapping1, tol);

  return QGaussSimplexExt<1>(degree).mapped_quadrature(vec_of_simplices);
}

template <>
Quadrature<2> compute_quadrature_on_intersection<2, 2, 2>(
    const typename Triangulation<2, 2>::cell_iterator &cell0,
    const typename Triangulation<2, 2>::cell_iterator &cell1,
    const unsigned int degree, const Mapping<2, 2> &mapping0,
    const Mapping<2, 2> &mapping1, const double tol) {
  const std::vector<std::array<Point<2>, 3>> &vec_of_simplices =
      CGALWrappers::compute_intersection_of_cells(cell0, cell1, mapping0,
                                                  mapping1, tol);
  return QGaussSimplexExt<2>(degree).mapped_quadrature(vec_of_simplices);
}

template <>
Quadrature<3> compute_quadrature_on_intersection<3, 1, 3>(
    const typename Triangulation<3, 3>::cell_iterator &cell0,
    const typename Triangulation<1, 3>::cell_iterator &cell1,
    const unsigned int degree, const Mapping<3, 3> &mapping0,
    const Mapping<1, 3> &mapping1, const double tol) {
...
template <>
Quadrature<3> compute_quadrature_on_intersection<3, 2, 3>(
    const typename Triangulation<3, 3>::cell_iterator &cell0,
    const typename Triangulation<2, 3>::cell_iterator &cell1,
    const unsigned int degree, const Mapping<3, 3> &mapping0,
    const Mapping<2, 3> &mapping1, const double tol) {
...
template <>
Quadrature<3> compute_quadrature_on_intersection<3, 3, 3>(
    const typename Triangulation<3, 3>::cell_iterator &cell0,
    const typename Triangulation<3, 3>::cell_iterator &cell1,
    const unsigned int degree, const Mapping<3, 3> &mapping0,
    const Mapping<3, 3> &mapping1, const double tol) {

  const std::vector<std::array<Point<3>, 4>> &vec_of_simplices =
      CGALWrappers::compute_intersection_of_cells(cell0, cell1, mapping0,
                                                  mapping1, tol);
  return QGaussSimplexExt<3>(degree).mapped_quadrature(vec_of_simplices);
}

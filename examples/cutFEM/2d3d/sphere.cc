...
template <typename Tester> class Discrete_signed_distance : public Function<3> {
  using K = CGAL::Exact_predicates_inexact_constructions_kernel;
  using Point_3 = K::Point_3;

public:
  Discrete_signed_distance(Tester tester) : is_in_tester(tester) {
  ...
  virtual double value(const Point<3> &p,
    ...
    if (*is_in_tester(CGALWrappers::dealii_point_to_cgal_point<Point_3, 3>(p)))
      return -1.;
    else
      return +1.;
  }
...
void LaplaceSolver<dim>::setup_discrete_level_set(const unsigned int cycle) {
  ...
  NonMatchingUtilities::DiscreteLevelSet<3, decltype(tree),
                                         Mesh_domain_poly::Is_in_domain>
